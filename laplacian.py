...
def corr(data, d_sub, name, what, array, R,n_label):
    ...
    dump_svmlight_file(np.around(train2,decimals=4), label, 'other_method/kmeans_linear/%s/%s_corr' % (name, what),
                       zero_based=False)
def test_transform(Rlaplacian,d_sub,name,what,data,tra_label):
...
def sampling( d_sub, name, what,tradeoff,n_label,data,tra_label):
    # test_url = "/home/comp/cszjlei/svm/BudgetedSVM/original/%s/train_%d" % (name, 0)
    # # train_url = "/home/comp/cszjlei/svm/BudgetedSVM/original/%s/train" % name
    # data, tra_label = load_svmlight_file(test_url)
    label = np.mat(tra_label)
    np.set_printoptions(threshold=np.inf, suppress=True)
    n_sample, d_feature = np.shape(data)
    # label = np.where(data[:, 0]!=1,-1,1)
    ...
    sub = np.vstack((a[positive[:n_label]],a[negitive[:n_label]])) #sub is feature 1600*d_feature
    ...
    trace = sorted(trace)
    # plt.scatter([x for x in range(len(topn[0]))], trace, linewidth=3, linestyle='--', marker='.')
    # plt.show()
    # exit()
...
def read_original(name,f_s,train,tra_label,test,te_label):
    ...
    R_laplacian = sampling(d_sub = sub,name  = name, what =  'train',tradeoff = trad, n_label = int(len(tra_label)/10),data=train,tra_label=tra_label)
    test_transform(Rlaplacian = R_laplacian,d_sub = sub,name  = name, what =  'test',data=test,tra_label=te_label)
...
if __name__ == '__main__':
    ...
    for idx, val in enumerate(method):
        np.save('other_method/kmeans_linear/%s/%s_result'%(name,'laplacian'),summary[idx])
    for m in method:
