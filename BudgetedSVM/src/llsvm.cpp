...
float predictLLSVM(budgetedData *testData, parameters *param, budgetedModelLLSVM *model, vector <int> *labels, vector <float> *scores)
{
	...
	temp =(*model).modelLLSVMmatrixW * (*model).modelLLSVMweightVector;
	//if ((*param).VERBOSE)
	...
	while (stillChunksLeft)
	{
		...
		for (unsigned int i = 0; i < N; i++)
		{
			...
			predictions(i) = v.transpose() * temp;
...
//    }
    ...
    return (float) (100.0 * (float) err / (float) total),(double) ((double)timeCalc / (double)CLOCKS_PER_SEC);
}
}
...
void trainLLSVM(budgetedData *trainData, parameters *param, budgetedModelLLSVM *model)
{
  ...
  unsigned long t1 = 0,t2 = 0,t3 = 0, start1,start2,start3;
...
//            t1 = clock() - start1;
//			sprintf(text, "time of  select landmark point is %f second\n",(double)t1/ (double)CLOCKS_PER_SEC);
//            svmPrintString(text);
//			if ((*param).VERBOSE)
//				svmPrintString("Computing the mapping function ...\n");
            ...
            t2 = clock() - start2;
		    sprintf(text, "time of  compute W is %f second\n",(double)t2/(double)CLOCKS_PER_SEC);
            svmPrintString(text);
		}
...
//		sprintf(text, "total time of  mapping training data is %f second\n",(double)t3/ (double)CLOCKS_PER_SEC);
//        svmPrintString(text);
//		if ((*param).VERBOSE)
