...
{	
    ...
    if (param.OUTPUT_SCORES)
        acc,time = predictLLSVM(testData, &param, (budgetedModelLLSVM*) model, &predLabels, &predScores);
    else
        acc,time = predictLLSVM(testData, &param, (budgetedModelLLSVM*) model, &predLabels);
