...
def random( d_sub, name, what, R,n_label,data,tra_label):
    ...
    # b = preprocessing.scale(data[:n_label],with_std=False)
    norm = np.linalg.norm(data, axis=0)
    p = norm**2/np.sum(norm**2)
    R_norm = np.random.choice(data.shape[1],d_sub,replace=False,p = p)
    ...
    dump_svmlight_file(np.around(train2,decimals=4), label, 'other_method/kmeans_linear/%s/%s_weight_sampling' % (name, what),
                       zero_based=False)
    # print(1)
    return R_norm,1/np.sqrt(d_sub*p[R_norm])

def test_transform(Rrandom,Rnorm,p,d_sub,name,what,data,tra_label):
...
def read_original(name,f_s,train,tra_label,test,te_label):
    ...
    R = sorted(R)
    # train1 = train
    R_norm ,p= random( d_sub=sub,name=name, what='train', R=R, n_label = int(len(tra_label)/2),data=train,tra_label=tra_label)
    test_transform(Rrandom=R,Rnorm=R_norm,p = p,d_sub = sub,name=name, what='test',data=test,tra_label=te_label)
...
if __name__ == '__main__':
    ...
    name = '%s'%(param.data)
    sub = 0
    acc = np.empty((1,len(f_sub),15,14)) #build the matrix to stall the result
    for f_idx,f_s in enumerate(f_sub):
        for n in range(15):
            train, train_lable, test, test_lable = generater(name,f_s)
    ...
    for idx, val in enumerate(method):
        numpy.save('other_method/kmeans_linear/%s/%s_result'%(name,'weight_sampling'),acc[idx])
        # print(acc[idx])
    for m in method:
